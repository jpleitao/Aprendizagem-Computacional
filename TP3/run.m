...
%%
%%  Main application's script, that starts the application's execution. This script includes all the code generated by Matlab for the
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    %Convert the results of the exectuion to an array of binary values
    network_results = convertResults(network_results);
    
    %Handle the classification, according to the defined type: Single Classification vs Group Classification
    if (strcmp(handles.classificationType,'group'))
        %Group classification type
        
        %The expected output for our neural network
        expected_output = translateOutputToGroup(handles.test_output, handles.groupLimitOnes, handles.window_size);
        
        %The output of our neural network
        got_output = translateOutputToGroup(network_results, handles.groupLimitOnes, handles.window_size);
        
        %Actually compare the output we got from the neural network's execution with the expected output for the given test set
